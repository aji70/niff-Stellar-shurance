use crate::{
    ...
    types::{
        AgeBand, CoverageType, Policy, PolicyType, PremiumQuote, RegionTier, RiskInput,
...
pub fn map_quote_error(env: &Env, err: Error) -> QuoteFailure {
    let message = match err {
        ...
        Error::RollingClaimCapExceeded => {
            "rolling claim cap exceeded for this policy in the current ledger window; reduce amount or wait for the next window"
        }
    };
    QuoteFailure {
        code: err as u32,
